...
export const orders=sqliteTable('orders',{id:text('id').primaryKey(),total:integer('total').notNull(),items:text('items').notNull(),token:text('token').notNull(),last4:text('last4').notNull(),expiry:text('expiry').notNull(),created:text('created').notNull()});
